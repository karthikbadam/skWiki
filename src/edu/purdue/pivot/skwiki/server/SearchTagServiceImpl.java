...
import edu.purdue.pivot.skwiki.client.SearchTagService;
...
public class SearchTagServiceImpl extends RemoteServiceServlet implements
SearchTagService {
	...
	@Override
	public DataPack searchTag(DataPack input) throws IllegalArgumentException {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM"+" tag where tag ~ "+
			"'"+input.searchedTag+"'");
			//int rowCount = 0;
			while (rs.next()) {
				...
				returnPack.searchTagList.add(tempResult);
			
			}
			
			
			System.out.println("sedarch tag list size "+returnPack.searchTagList.size());
